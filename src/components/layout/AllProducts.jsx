    ...
    return (
        <div className="allProducts">
            ...
            <div className="allProducts__body">
                <img className="allProducts__body__filters" src={Filters}/>
                <div className="allProducts__body__list">
                    {allItems.map((item, index) => {
                        return (
                            <div className="allProducts__body__list__details" key={index}>
                                <div className="allProducts__body__list__details__picture"><img src={item.img} alt={item.name} /></div>
                                <div className="allProducts__body__list__details__brand">{item.brand !== "" ? <img src={item.brand} alt="marque" /> : ""}</div>
                                <div className="allProducts__body__list__details__name"><p>{item.name}</p></div>
                                <div className="allProducts__body__list__details__price">
                                    <div className="allProducts__body__list__details__price__HT">{item.price}<p>€00 HT</p></div>
                                    <div className="allProducts__body__list__details__price__TTC">{item.price * 20 / 100 + item.price} € TTC <img src={Mpro} alt="Mpro"/></div>
                                </div>
                            </div>
                        )
                    })}
